...
import biz.netcentric.util.ParserUtilities;
import javax.servlet.http.HttpServletRequest;

/**
 * Parser HTML implementing a small server-side html templating language with some server-side scripting in javascript.
 * 
 * @author Eduard Reyes
 */
public class HTMLJsoupParser {
	 ...
	 * Parses HTML and Javascript code from a template
         * @param httpRequest httpRequest from filter
         * @return 
         */
	public String doParse(String url, String realPath){
		Document doc = null;
		try {                        
                        File input = new File(realPath);
			doc = Jsoup.parse(input, "UTF-8");
                        parseJavascript(doc, url, realPath);
			parseIf(doc);
			parseForChild(doc);
	 ...
	 */
	private void parseJavascript(Document doc, String url, String realPath) throws ScriptException, FileNotFoundException, InstantiationException, IllegalAccessException{
